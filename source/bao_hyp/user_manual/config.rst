...
If you intend to use built-in guest images (that are embedded in the hypervisor), you can
**optionally** declare them using the ``VM_IMAGE`` macro. For details and usage, please refer to
the `Guest Image`_ section.

VM Configuration
----------------

Bao's configuration file allows you to partition the platform’s hardware resources, including CPU
cores, memory regions, devices, and communication channels, across multiple VMs according to your
needs. In Bao, CPU cores, memory regions, and interrupts are always assigned exclusively to a
single VM and cannot be shared. In contrast, certain hardware devices and communication resources
...
configuration of each individual VM:
...
Bao supports two primary methods for providing VM images: **Built-in Images** and **Separately
Loaded** Images. The choice of method determines how the image is made available to the hypervisor
and how it is loaded into the VM’s memory at boot.

1. **Built-in images.** The guest image is embedded within the Bao hypervisor binary at build time.
This is simple and convenient, but may not always be optimal for large images (also depending on
the target architecture, which might have faster memories to place guest images for faster boot).

2. **Separately Loaded Images** are not embedded within the Bao hypervisor image. Instead, these
are provided by an external bootloader or a similar mechanism at system boot. This approach can
speed up boot and optimize system memory usage by allowing the image to be used “in place” without
copying.

If you use built-in images, you can **optionally** declare each VM image at the beginning of your
configuration file using the ``VM_IMAGE`` macro:
...
1. The ``img_name``, a unique identifier associated with the image that will later be used to
...
- ``inplace`` [optional]- use the image inplace and don’t copy the image. By default, ``inplace``
  is set as ``false``;
...
1. ``VM_IMAGE_BUILTIN`` - This macro simplifies image configuration by requiring only the
   ...
   size, which simplifies system configuration by leveraging Bao's default knowledge of the
   image location. No separate configuration or loading of VM images through a bootloader is
   required, and adjustments to the size of VM images are unnecessary.

2. ``VM_IMAGE_LOADED`` - This macro requires additional configurations. It requires the definition
   of image ``base_addr``, the image ``load_addr``, and the image ``size``. The use of this macro
   is highly recommended, especially for MPU systems, where manual allocation of space for the VM
   image can be challenging if embedded in Bao's binary. Without utilizing ``VM_IMAGE_LOADED``,
   Bao may need to copy the image, potentially resulting in wasted space.

.. note::
  The integration of the appropriate macro, tailored to your specific use case, is crucial for
  ensuring consistency and reliability in your VM setup. **Manual configuration of the**
  ``vm_image`` **struct is discouraged**. Always use the appropriate macro for your deployment
  scenario. This ensures correct alignment, address calculations, and compatibility with different
  Bao versions and hardware platforms.
...
In Bao’s configuration, you will encounter both **guest addresses** and **physical addresses**
when specifying memory regions:

- **Guest Addresses**: This is the address as seen by the guest operating system running inside
  the VM. It is the location where the guest “believes” its memory resides and is used by the
  guest OS and applications.

- **Physical Addresses**: This is the address in the actual hardware physical address space of
  the platform (i.e., main memory as seen by the processor and peripherals). The hypervisor uses
  this address to control actual memory allocation and resource mapping.

Typically, the hypervisor maps each guest memory region, configured with a guest address, to a
specific physical address on the platform. On systems with an MMU, these can be different. On
MPU-based systems or systems without virtualization support, they are often the same.
...
- ``phys`` [mandatory if ``place_phys`` is ``true``] - it corresponds to the physical address
  where the memory region should be mapped;
...
- ``interrupts`` [mandatory if ``interrupt_num`` > 0] - defines a list of interrupt IDs assigned
  to the IPC - ``(irqid_t[]) {irq_1, ..., irq_n}``;
...
- ``va`` [mandatory] - corresponds to the base guest ``virtual address`` of the device;
...
.. tabs::
  .. tab:: ARM Platforms
    If the platform integrates an SMMU (System Memory Management Unit), the
    ``id`` field specifies the StreamID that the IOMMU will use to tag DMA
    transaction requests from this device. Proper configuration is critical for
    isolating device access in the guest memory map. For simple SoCs or those
    without SMMU, this field is typically zero or ignored.

  .. tab:: RISC-V Platforms
    If IOMMU functionality is present, ``id`` configures the StreamID used in
    IOMMU translation for the device, enabling fine-grained DMA remapping and
    device assignment to specific VMs. If there is no IOMMU, this field is
    ignored.

  .. tab:: MPU and simpler embedded platforms
    The ``id`` field is generally ignored, as hardware typically does not
    support DMA translation or bus master remapping. Device mappings follow a
    1:1 mapping scheme.
...
- ``base / phys`` [optional] - When ``place_phys`` is set to ``false``, the ``base`` parameter \
  should be used to specify the VM virtual address of the shared memory. When ``place_phys`` is \
